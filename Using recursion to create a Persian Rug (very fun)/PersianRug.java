...
public class PersianRug {
	...
	private static void persianRug(
		...
		int newColor = ((north + east + south + west) * 1 + 1)% palette.length;
		
		StdDraw.setPenColor(palette[newColor]);
		StdDraw.line(llx, lly+size/2, llx + size, lly + size/2);
		StdDraw.line(llx + size/2, lly, llx + size/2, lly + size);
		persianRug(palette,llx, lly, size/2, newColor, newColor, south, west);  //lower left
	    persianRug(palette,llx, lly + size/2, size/2, north, newColor, newColor, west);  //upper left
		persianRug(palette, llx + size/2, lly+ size/2, size/2, north, east, newColor, newColor); //upper right
		persianRug(palette, llx+ size/2, lly, size/2, newColor, east, south, newColor);
	}
